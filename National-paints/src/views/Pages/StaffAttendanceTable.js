...
const StaffAttendanceTable = () => {
  ...
  const [dailyConsecutiveDays, setDailyConsecutiveDays] = useState(0);
  ...
  var consecutiveDailyLateDays = 0;
  ...
  const getAttendance = (attendanceRecord, day) => {
    ...
    if (attendanceRecord?.timeLogs?.length > 0) {
      ...
      // Half day for 3 consecutive late days
  ...
  const calculateTotalLeaves = (attendanceRecords) => {
    ...
    daysInMonth.forEach((day) => {
      ...
      // Count as leave only for past regular working days with no attendance
      if (
        ...
        <Button colorScheme="green" display={"flex"} gap={3} mb={4}>
          <CSVLink
            data={csvData?.map((employee) => ({
              ...
              totalHalfDays: employee.totalHalfDays,
