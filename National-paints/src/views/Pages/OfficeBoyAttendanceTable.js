...
const OfficeBoyAttendanceTable = () => {
  ...
  const getDailyHours = (attendanceRecord, day) => {
    ...
    if (!attendanceRecord) {
      ...
      // For past days (Sunday/Holiday): show 8:0 by default
    ...
    // Check if the day has been removed (cross button clicked)
    ...
    // If no check-in/check-out (employee didn't work that day)
    if (!checkInTime || !checkOutTime) {
      // If it's a holiday/Sunday and not removed, give 8 hours (only for past days)
    ...
    // Employee has check-in/check-out (actually worked that day)
    ...
    // Don't add extra 8 hours if employee actually worked - they get paid for actual overtime hours
  ...
  const calculateTotalOvertime = (attendanceRecords) => {
    ...
    daysInMonth?.forEach((day) => {
      const currentDate = new Date(year, month, day);
      const dayString = currentDate.toLocaleDateString("en-CA");
      ...
      // Calculate overtime for all days with attendance (including regular days)
      ...
      >
        ...
        <Button colorScheme="green" display={"flex"} gap={3} mb={4}>
          <CSVLink
            ...
            filename={`officeboyAttendance.csv`}
                ...
                {daysInMonth.map((day) => {
                  const attendanceRecord = employee.attendanceTime.find(
                    (record) => {
                      ...
                      return (
                        recordDate.split("-", 3)[2] === String(day).padStart(2, '0') &&
                        new Date(record.date).getFullYear() === year &&
...
export default OfficeBoyAttendanceTable;